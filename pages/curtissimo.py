# curtissimo.PY - INTERFACE VISUAL (LEITURA/INSERÇÃO NA SUPABASE; SEM ALTERAR/DELETAR NA NUVEM)
# -*- coding: utf-8 -*-
...
# -----------------------------
# SUPABASE (LEITURA/INSERÇÃO EM kv_state_curtissimo)
# -----------------------------
...
SUPABASE_TABLE = "kv_state_curtissimo"
STATE_KEY      = "curtissimo_przo_v1"   # (atenção: é "przo" mesmo)
...
def ler_ativos_da_supabase() -> list[dict]:
    """
    Lê os ativos de v['ativos'] na linha (k) 'curtissimo_przo_v1' da tabela kv_state_curtissimo.
...
def inserir_ativo_na_supabase(ticker: str, operacao: str, preco: float) -> tuple[bool, str | None]:
    """
    Insere um novo ativo no array v['ativos'] (merge) da chave 'curtissimo_przo_v1'.
...
VIS_STATE_FILE = "session_data/visual_state_curtissimo.json"
...
def limpar_tabela_supabase():
    """
    Apaga todos os dados da chave curtissimo_przo_v1 na tabela kv_state_curtissimo (Supabase).
...
# -----------------------------
now = agora_lx()
st.title("⚡ CURTÍSSIMO PRAZO - COMPRA E VENDA")
...
with st.expander("ℹ️ Como funciona esta interface?"):
    st.markdown("""
- **Leitura** de ativos é feita em **Supabase → kv_state_curtissimo**, linha `k="curtissimo_przo_v1"`, lendo `v["ativos"]`.  
