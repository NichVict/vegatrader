# LOSS_curtissimo.PY - INTERFACE VISUAL (LEITURA/INSERÇÃO NA SUPABASE; SEM ALTERAR/DELETAR NA NUVEM)
# -*- coding: utf-8 -*-
...
# -----------------------------
# SUPABASE (LEITURA/INSERÇÃO EM kv_state_losscurtissimo)
# -----------------------------
...
SUPABASE_TABLE = "kv_state_losscurtissimo"
...
def ler_ativos_da_supabase() -> list[dict]:
    """
    Lê os ativos de v['ativos'] na linha (k) 'loss_curtissimo_przo_v1' da tabela kv_state_losscurtissimo.
...
def inserir_ativo_na_supabase(ticker: str, operacao: str, preco: float) -> tuple[bool, str | None]:
    """
    Insere novo ativo no array v['ativos'] (merge) da chave 'loss_curtissimo_przo_v1'.
...
VIS_STATE_FILE = "session_data/visual_state_loss_curtissimo.json"
...
# -----------------------------
st.sidebar.header("⚙️ Configurações (LOSS CURTÍSSIMO)")
...
async def testar_telegram():
    ...
    try:
        if tok and chat:
            bot = Bot(token=tok)
            await bot.send_message(chat_id=chat, text="✅ Teste de alerta LOSS curtissimo funcionando!")
            return True, None
...
def limpar_tabela_supabase():
    """
    Apaga todos os dados da chave loss_curtissimo_przo_v1 na tabela kv_state_losscurtissimo (Supabase).
...
with st.expander("ℹ️ Como funciona esta interface (LOSS)?"):
    st.markdown("""
- **Leitura** de STOPs é feita em **Supabase → kv_state_losscurtissimo**, linha `k="loss_curtissimo_przo_v1"`, lendo `v["ativos"]`.  
...
- O robô na nuvem (loss_curtissimo) é quem efetivamente envia os alertas reais de encerramento.
    """)
