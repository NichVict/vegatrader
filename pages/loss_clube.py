...
# CONFIGURAÇÕES
# -----------------------------
st.set_page_config(page_title="🛑 LOSS CLUBE - ENCERRAMENTO POR STOP", layout="wide")
...
# Identificador único deste app (evita colisão com outros scripts)
APP_ID = "loss_clube"
...
STATE_KEY = "loss_clube_przo_v1"
LOCAL_STATE_FILE = "session_data/state_lossclube.json"
...
def enviar_notificacao_clube(dest, assunto, corpo_email_html, rem, senha, tok_tg, chat_id, corpo_telegram=None):
    """
...
# -----------------------------
# MENSAGENS DE ENCERRAMENTO (LOSS clube)
...
def formatar_mensagem_encerramento(ticker_symbol, preco_alvo, preco_atual, operacao):
    ...
    mensagem_telegram = (
        ...
        f"<em>"
        f"COMPLIANCE: Esta mensagem é uma sugestão de ENCERRAMENTO baseada na CARTEIRA CLUBE. "
...
<html>
  <body style="font-family:Arial,sans-serif; background-color:#0b1220; color:#e5e7eb; padding:20px;">
    <h2 style="color:#ef4444;">ALERTA STOP CARTEIRA CLUBE</h2>
    ...
    <p style="font-size:11px; line-height:1.4; color:#9ca3af;">
      <b>COMPLIANCE:</b> Esta mensagem é uma sugestão de ENCERRAMENTO baseada na CARTEIRA CLUBE.<br>
...
def notificar_preco_alvo_alcancado_loss(ticker, preco_alvo, preco_atual, operacao):
    """
    Envia mensagens de ENCERRAMENTO (STOP) no mesmo padrão visual do clube.
    """
    ...
    destinatario = st.secrets.get("email_recipient_lossclube", "")
    token_tg = st.secrets.get("telegram_token", "")
    chat_id = st.secrets.get("telegram_chat_id_lossclube", "")

    try:
        enviar_notificacao_clube(destinatario, assunto, msg_email_html, remetente, senha, token_tg, chat_id, msg_telegram)
...
async def testar_telegram():
    ...
    chat = st.secrets.get("telegram_chat_id_lossclube", "")
    try:
        if tok and chat:
            bot = Bot(token=tok)
            await bot.send_message(chat_id=chat, text="✅ Teste de ENCERRAMENTO (LOSS CLUBE) funcionando!")
            return True, None
...
def notificar_abertura_pregao_uma_vez_por_dia():
    ...
    try:
        ...
        chat = st.secrets.get("telegram_chat_id_lossclube", "").strip()
        if tok and chat:
            bot = Bot(token=tok)
            asyncio.run(bot.send_message(chat_id=chat, text="🛑 Robô LOSS CLUBE ativo — Pregão Aberto!"))
...
# -----------------------------
now = agora_lx()
st.title("🛑 LOSS CLUBE - ENCERRAMENTO POR STOP")
...
st.write("Robô automático da **CARTEIRA CLUBE (LOSS)** — envia **ENCERRAMENTO (STOP)** após **15 min** na zona de preço alvo.")
...
if st.session_state.pausado:
    ...
    if dentro_pregao(now):
        ...
        for ativo in st.session_state.ativos:
            ...
            # Mantemos a mesma regra de "zona" do clube:
            ...
            if condicao:
                ...
                if (
                    ...
                    # id: app + ticker + ação + alvo + data
                    event_id = f"{APP_ID}|{t}|{operacao_atv}|{preco_alvo:.2f}|{now.date()}"
                    
                    # já enviou hoje? -> ignora
                    if st.session_state["eventos_enviados"].get(event_id):
                        st.session_state.log_monitoramento.append(
                            f"🔁 {t}: envio ignorado (já enviado hoje)."
                        )
                    else:
                        # 1) MARCA E PERSISTE ANTES DE ENVIAR (corta duplicidade por reexecução)
...
with st.expander("🧪 Debug / Backup do estado (JSON)", expanded=False):
    try:
        ...
        if res.status_code == 200 and res.json():
            ...
            st.download_button("⬇️ Baixar state_loss_clube.json",
                               data=json.dumps(state_preview, indent=2),
                               file_name="state_loss_clube.json", mime="application/json")
        else:
...
st_autorefresh(interval=refresh_ms, limit=None, key="loss-clube-refresh")
