# LOSS_clube.PY - INTERFACE VISUAL (LEITURA/INSERÇÃO NA SUPABASE; SEM ALTERAR/DELETAR NA NUVEM)
# -*- coding: utf-8 -*-
...
# -----------------------------
# SUPABASE (LEITURA/INSERÇÃO EM kv_state_lossclube)
# -----------------------------
...
SUPABASE_TABLE = "kv_state_lossclube"
...
def ler_ativos_da_supabase() -> list[dict]:
    """
    Lê os ativos de v['ativos'] na linha (k) 'loss_clube_przo_v1' da tabela kv_state_lossclube.
...
def inserir_ativo_na_supabase(ticker: str, operacao: str, preco: float) -> tuple[bool, str | None]:
    """
    Insere novo ativo no array v['ativos'] (merge) da chave 'loss_clube_przo_v1'.
...
VIS_STATE_FILE = "session_data/visual_state_loss_clube.json"
...
# -----------------------------
st.sidebar.header("⚙️ Configurações (LOSS CLUBE)")
...
async def testar_telegram():
    ...
    try:
        if tok and chat:
            bot = Bot(token=tok)
            await bot.send_message(chat_id=chat, text="✅ Teste de alerta LOSS CLUBE funcionando!")
            return True, None
...
def limpar_tabela_supabase():
    """
    Apaga todos os dados da chave loss_clube_przo_v1 na tabela kv_state_lossclube (Supabase).
...
if st.sidebar.button("🧹 Limpar Banco de Dados (LOSS)"):
    ...
    if ok:
        st.sidebar.success("✅ Tabela limpa com sucesso na Supabase (LOSS)!")
        st.session_state.log_monitoramento.append(
            f"{agora_lx().strftime('%H:%M:%S')} | LIMPAR TABELA (LOSS) executado com sucesso."
        )
    else:
...
with st.expander("ℹ️ Como funciona esta interface (LOSS)?"):
    st.markdown("""
- **Leitura** de STOPs é feita em **Supabase → kv_state_lossclube**, linha `k="loss_clube_przo_v1"`, lendo `v["ativos"]`.  
...
- O robô na nuvem (loss_clube) é quem efetivamente envia os alertas reais de encerramento.
    """)
