...
# CONFIGURAÇÕES
# -----------------------------
st.set_page_config(page_title="🛑 LOSS CURTO - ENCERRAMENTO POR STOP", layout="wide")
...
# -----------------------------
# MENSAGENS DE ENCERRAMENTO (LOSS CURTO)
...
def formatar_mensagem_encerramento(ticker_symbol, preco_alvo, preco_atual, operacao):
    ...
    mensagem_telegram = (
        ...
        f"<em>"
        f"COMPLIANCE: Esta mensagem é uma sugestão de ENCERRAMENTO baseada na CARTEIRA CURTO PRAZO. "
...
<html>
  <body style="font-family:Arial,sans-serif; background-color:#0b1220; color:#e5e7eb; padding:20px;">
    <h2 style="color:#ef4444;">ALERTA STOP CARTEIRA CURTO PRAZO</h2>
...
def notificar_preco_alvo_alcancado_loss(ticker, preco_alvo, preco_atual, operacao):
    """
    Envia mensagens de ENCERRAMENTO (STOP) no mesmo padrão visual do curto.
    """
    ...
    destinatario = st.secrets.get("email_recipient_losscurto", "")
...
async def testar_telegram():
    ...
    try:
        if tok and chat:
            bot = Bot(token=tok)
            await bot.send_message(chat_id=chat, text="✅ Teste de ENCERRAMENTO (LOSS CURTO) funcionando!")
            return True, None
...
def notificar_abertura_pregao_uma_vez_por_dia():
    ...
    try:
        ...
        if tok and chat:
            bot = Bot(token=tok)
            asyncio.run(bot.send_message(chat_id=chat, text="🛑 Robô LOSS CURTO ativo — Pregão Aberto!"))
...
# -----------------------------
now = agora_lx()
st.title("🛑 LOSS CURTO - ENCERRAMENTO POR STOP")
...
st.write("Robô automático da **CARTEIRA CURTO PRAZO (LOSS)** — envia **ENCERRAMENTO (STOP)** após **15 min** na zona de preço alvo.")
...
if st.session_state.pausado:
    ...
    if dentro_pregao(now):
        ...
        for ativo in st.session_state.ativos:
            ...
            # Mantemos a mesma regra de "zona" do curto:
...
with st.expander("🧪 Debug / Backup do estado (JSON)", expanded=False):
    try:
        ...
        if res.status_code == 200 and res.json():
            ...
            st.download_button("⬇️ Baixar state_loss_curto.json",
                               data=json.dumps(state_preview, indent=2),
                               file_name="state_loss_curto.json", mime="application/json")
        else:
...
st_autorefresh(interval=refresh_ms, limit=None, key="loss-curto-refresh")
