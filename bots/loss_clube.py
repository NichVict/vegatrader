...
def _write_heartbeat():
    """Grava o heartbeat direto no Supabase (independente do painel central)."""
    try:
        # Nome da tabela correspondente a este robô:
        ...
        payload = {"k": "heartbeat_loss_clube", "v": {"ts": now}}
        requests.post(url, headers=headers, json=payload, timeout=10)
        return True
    except Exception as e:
        print(f"[loss_clube] Erro ao gravar heartbeat: {e}")
        return False
